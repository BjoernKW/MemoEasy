require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe StaffMembersController do

  login_user
  render_views

  before(:each) do
    @staff_member = FactoryGirl.create(:staff_member, :company => @company)
  end


  # This should return the minimal set of attributes required to create a valid
  # StaffMember. As you add validations to StaffMember, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    attributes_for(:staff_member)
  end

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # StaffMembersController. Be sure to keep this updated too.
  def valid_session
    {}
  end

  describe "GET index" do
    it "assigns all staff_members as @staff_members" do
      get :index, {}
      expect(assigns(:staff_members)).to eq([@staff_member])
    end
  end

  describe "GET show" do
    it "assigns the requested staff_member as @staff_member" do
      get :show, {:id => @staff_member.to_param}
      expect(assigns(:staff_member)).to eq(@staff_member)
    end
  end

  describe "GET new" do
    it "assigns a new staff_member as @staff_member" do
      get :new, {}
      expect(assigns(:staff_member)).to be_a_new(StaffMember)
    end
  end

  describe "GET edit" do
    it "assigns the requested staff_member as @staff_member" do
      get :edit, {:id => @staff_member.to_param}
      expect(assigns(:staff_member)).to eq(@staff_member)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new StaffMember" do
        expect {
          post :create, {:staff_member => valid_attributes}
        }.to change(StaffMember, :count).by(1)
      end

      it "assigns a newly created staff_member as @staff_member" do
        post :create, {:staff_member => valid_attributes}
        expect(assigns(:staff_member)).to be_a(StaffMember)
        expect(assigns(:staff_member)).to be_persisted
      end

      it "redirects to staff_member list" do
        post :create, {:staff_member => valid_attributes}
        expect(response).to redirect_to(staff_members_url)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved staff_member as @staff_member" do
        # Trigger the behavior that occurs when invalid params are submitted
        StaffMember.any_instance.stub(:save).and_return(false)
        post :create, {:staff_member => {  }}
        expect(assigns(:staff_member)).to be_a_new(StaffMember)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        StaffMember.any_instance.stub(:save).and_return(false)
        post :create, {:staff_member => {  }}
        expect(response).to render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested staff_member" do
        # Assuming there are no other staff_members in the database, this
        # specifies that the StaffMember created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        StaffMember.any_instance.should_receive(:update_attributes).with({ "these" => "params" })
        put :update, {:id => @staff_member.to_param, :staff_member => { "these" => "params" }}
      end

      it "assigns the requested staff_member as @staff_member" do
        put :update, {:id => @staff_member.to_param, :staff_member => valid_attributes}
        expect(assigns(:staff_member)).to eq(@staff_member)
      end

      it "redirects to the staff_member" do
        put :update, {:id => @staff_member.to_param, :staff_member => valid_attributes}
        expect(response).to redirect_to(@staff_member)
      end
    end

    describe "with invalid params" do
      it "assigns the staff_member as @staff_member" do
        # Trigger the behavior that occurs when invalid params are submitted
        StaffMember.any_instance.stub(:save).and_return(false)
        put :update, {:id => @staff_member.to_param, :staff_member => {  }}
        expect(assigns(:staff_member)).to eq(@staff_member)
      end

      it "re-renders the 'edit' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        StaffMember.any_instance.stub(:save).and_return(false)
        put :update, {:id => @staff_member.to_param, :staff_member => {  }}
        expect(response).to render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested staff_member" do
      expect {
        delete :destroy, {:id => @staff_member.to_param}
      }.to change(StaffMember, :count).by(-1)
    end

    it "redirects to the staff_members list" do
      delete :destroy, {:id => @staff_member.to_param}
      expect(response).to redirect_to(staff_members_url)
    end
  end

end
